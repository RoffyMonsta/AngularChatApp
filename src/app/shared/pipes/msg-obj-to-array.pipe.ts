...
currentUserName: string = JSON.parse(localStorage.getItem('user')).email;
  transform(messageObj: {
    ...
    for(let p in messageObj) { 
      ...
      let msgSrc = messageObj[p].source === 'user' ? `${this.currentUserName}, ${format_time}` : `${userName}, ${format_time}`;
